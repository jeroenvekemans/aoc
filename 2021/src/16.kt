...
object DaySixteen {
    ...
    private fun solve(
        ...
        literalResultComputer: (Int, Long) -> Long,
        operandResultComputer: (Int, Int, List<Long>) -> Long
    ): Pair<Long, String> {
        ...
        if (packetId == 4) {
            ...
            return Pair(literalResultComputer(packetVersion, literalValue), binaryRepresentation.substring(6 + 5 * (1 + firstZeroIndex)))
        } else {
            ...
            if (operatorPacket == "0") {
                ...
                while (remainder.isNotEmpty()) {
                    val solution = solve(remainder, literalResultComputer, operandResultComputer)
                ...
                return Pair(
                    operandResultComputer(packetVersion, packetId, results),
                ...
                return Pair(operandResultComputer(packetVersion, packetId, results), remainder)
            }
        }
    }

    fun solve(encodedTransmission: String): Long {
        val binaryRepresentation = hexToBinary(encodedTransmission)

        return solve(binaryRepresentation, { packetVersion, _ -> packetVersion.toLong() }) { packetVersion, _, results ->
            packetVersion + results.sum()
        }.first
    }
    ...
    fun solveDelta(encodedTransmission: String): Long {
        ...
        return solve(binaryRepresentation, { _, literal -> literal}) { _, packetId, results ->
            when (packetId) {
                0 -> results.sum()
